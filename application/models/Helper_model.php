...
return $ossz;
		...
		public function getSavedPlansList()
		{
			...
			if($savenum==0)
			{
			$sql="SELECT * FROM kltsg_submissions_bevetel_saved
			WHERE  user_id='".$_COOKIE['userid']."'";
			$res=$this->db->query($sql) or die("Hiba a kltsg_submissions_bevetel_saved lekérdezésénél ");
			//echo $sql;
			$savenum = $res->num_rows();
			}
			//echo $savenum."síjt";
			...
			echo '<tr><td>'.$i++.'</td><td colspan=""><select id="inst_unit_'.$i.'" class="form-control">';
				...
				//Bevétele ellőnörzése, hátha csak azt vittek fel.
					$sql="SELECT submissions_id, (SELECT created_time FROM kltsg_submissions WHERE id=submissions_id) AS submissions_time,
			institute_id, unit_id, (SELECT name FROM kltsg_institute WHERE id=institute_id) AS institute_name,
			(SELECT name FROM kltsg_unit WHERE id=unit_id) AS unit_name FROM `kltsg_submissions_bevetel_saved` WHERE user_id='".$_COOKIE['userid']."' 
			GROUP BY unit_id,
			institute_id, submissions_id ORDER BY id DESC;";
			$category=$this->db->query($sql) or die("Hiba a kltsg  lekérdezésénél " . $this->db->error());
			...
			{
				$ok=1;
				for($j=0;$j<count($submission_id);$j++)
				{
					if($submission_id[$j]==$record->submissions_id)
					{
						$ok=0;
						break;
					}
